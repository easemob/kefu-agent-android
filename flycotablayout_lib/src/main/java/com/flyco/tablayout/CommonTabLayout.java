...
import com.example.flycotablayout_lib.R;
...
import static com.example.flycotablayout_lib.R.id.iv_tab_icon;

/**
 * 没有继承HorizontalScrollView 不能滑动， 对于ViewPager 无依赖
 **/
public class CommonTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
    ...
    public CommonTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (height.equals(ViewGroup.LayoutParams.MATCH_PARENT + "")) {
            ...
            mHeight = a.getDimensionPixelSize(0, -2);
            a.recycle();
        }
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        else mTabPadding = ta.getDimension(R.styleable.CommonTabLayout_tl_tab_padding, dp2px(10));
    ...
    private void addTab(final int position, View tabView) {
        ...
        CustomTabEntity entty = mTabEntitys.get(position);
        tv_tab_title.setText(entty.getTabTitle());
        if (entty.getId() > 0){
            tabView.setId(entty.getId());
        }
...
//                    iconH = mContext.getResources().getDrawable(mTabEntitys.get(position).getTabSelectedIcon()).getIntrinsicHeight();
                    iconH = ContextCompat.getDrawable(mContext, mTabEntitys.get(position).getTabSelectedIcon()).getIntrinsicHeight();
                }
                margin = mIconMargin;
            }
