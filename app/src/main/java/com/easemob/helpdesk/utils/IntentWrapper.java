...
//    public static final int XIAOMI_GOD = 102;
    ...
    //魅族  白名单
    public static final int MEIZU_WHITE = 118;
    ...
    static {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            PowerManager pm = (PowerManager) HDApplication.getInstance().getSystemService(Context.POWER_SERVICE);
            boolean ignoringBatteryOptimizations = pm.isIgnoringBatteryOptimizations(HDApplication.getInstance().getPackageName());
            if (!ignoringBatteryOptimizations) {
                Intent dozeIntent = new Intent(Settings.ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS);
                dozeIntent.setData(Uri.parse("package:" + HDApplication.getInstance().getPackageName()));
...
//        INTENT_WRAPPER_LIST.add(new IntentWrapper(xiaomiGodIntent, XIAOMI_GOD));
        ...
        //三星 5.0/5.1 自启动应用程序管理
        Intent samsungLIntent = HDApplication.getInstance().getPackageManager().getLaunchIntentForPackage("com.samsung.android.sm");
        if (samsungLIntent != null)
            INTENT_WRAPPER_LIST.add(new IntentWrapper(samsungLIntent, SAMSUNG_L));
        ...
        meizuIntent.putExtra("packageName", HDApplication.getInstance().getPackageName());
        ...
        //魅族 白名单
        Intent meizuWhiteIntent = new Intent();
        meizuWhiteIntent.setClassName("com.meizu.safe", "com.meizu.safe.ramcleaner.RAMCleanerActivity");
        meizuWhiteIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        INTENT_WRAPPER_LIST.add(new IntentWrapper(meizuWhiteIntent, MEIZU_WHITE));
    ...
    public boolean doesActivityExist() {
        PackageManager pm = HDApplication.getInstance().getPackageManager();
    ...
    public static String getApplicationName() {
        ...
        try {
            packageManager = HDApplication.getInstance().getPackageManager();
            applicationInfo = packageManager.getApplicationInfo(HDApplication.getInstance().getPackageName(), 0);
    ...
    public final static class MIUIUtils {
