 ...
 * {@link ImageCache2#getInstance(android.support.v4.app.FragmentManager, ImageCacheParams)}
...
public class ImageCache2 {
	...
	/**
	 * Create a new ImageCache2 object using the specified parameters. This
	 * should not be called directly by other classes, instead use
	 * {@link ImageCache2#getInstance(android.support.v4.app.FragmentManager, ImageCacheParams)}
	 * to fetch an ImageCache2 instance.
	 *
	 * @param cacheParams
	 *            The cache parameters to use to initialize the cache
	 */
	private ImageCache2(ImageCacheParams cacheParams) {
		init(cacheParams);
	}

	/**
	 * Return an {@link ImageCache2} instance. A {@link RetainFragment} is used
	 * to retain the ImageCache2 object across configuration changes such as a
	 ...
	 *            fragment.
	 * @param cacheParams
	 *            The cache parameters to use if the ImageCache2 needs
	 *            instantiation.
	 * @return An existing retained ImageCache2 object or a new one if one did
	 *         not exist
	 */
	public static ImageCache2 getInstance(FragmentManager fragmentManager,
	                                     ImageCacheParams cacheParams) {

		// Search for, or create an instance of the non-UI RetainFragment
		final RetainFragment mRetainFragment = findOrCreateRetainFragment(fragmentManager);

		// See if we already have an ImageCache2 stored in RetainFragment
		ImageCache2 imageCache = (ImageCache2) mRetainFragment.getObject();

		// No existing ImageCache2, create one and store it in RetainFragment
		if (imageCache == null) {
			imageCache = new ImageCache2(cacheParams);
...
//		}
	...
	/**
	 * Clears both the memory and disk cache associated with this ImageCache2
...
//		}
	 ...
	 * configuration changes. It will be used to retain the ImageCache2 object.
	 */
