...
public class FaceConversionUtil {
	...
	public SpannableString addFace(Context context, int imgId,
		...
		Bitmap bitmap = BitmapFactory.decodeResource(context.getResources(),
				imgId);
//		bitmap = Bitmap.createScaledBitmap(bitmap, CommonUtils.convertDip2Px(context,20), CommonUtils.convertDip2Px(context, 20), true);
		int height = DisplayUtil.sp2px(context, 20);
...
//				Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
	...
	private void dealExpression(Context context,
		...
		while (matcher.find()) {
			...
			if (resId != 0) {
				...
				bitmap = Bitmap.createScaledBitmap(bitmap, CommonUtils.convertDip2Px(context, 22), CommonUtils.convertDip2Px(context, 22), true);
	...
	private List<ChatEmoji> getData(int page) {
		...
		if (list.size() == pageSize) {
			ChatEmoji object = new ChatEmoji();
			object.setId(R.drawable.icon_face_delete_2);
			list.add(object);
		}
		return list;
	}
