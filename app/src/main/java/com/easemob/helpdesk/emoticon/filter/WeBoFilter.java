 ...
 */
public class WeBoFilter extends EmoticonFilter {

    public static final int WRAP_DRAWABLE = -1;
    private int emoticonSize = -1;
    public static final Pattern WeBo_RANGE = Pattern.compile("\\[[a-zA-Z0-9\\u4e00-\\u9fa5]+\\]");

    public static Matcher getMatcher(CharSequence matchStr){
        return WeBo_RANGE.matcher(matchStr);
    }
    ...
    public void filter(EditText editText, CharSequence text, int start, int lengthBefore, int lengthAfter) {
        ...
        while (m.find()){
            String key = m.group();
            String icon = DefWeiBoEmoticons.wbEmoticonHashMap.get(key);
