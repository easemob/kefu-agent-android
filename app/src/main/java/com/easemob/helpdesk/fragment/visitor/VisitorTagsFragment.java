...
//    private TagAdapter<String> mAdapter;
    ...
    private void getUserTagsFromRemote(){
        //获取用户标签
        VisitorManager.getInstance().getUserTag(visitorId, new HDDataCallBack<List<UserTag>>() {
            @Override
        ...
        mFlowLayout.setOnTagClickListener(new TagFlowLayout.OnTagClickListener() {
            ...
            public boolean onTagClick(View view, int position, FlowLayout parent) {
                ...
                if(checkedPosition.contains(position)){
                    VisitorManager.getInstance().setTag(true, visitorId, userTag);
                }else {
                    VisitorManager.getInstance().setTag(false, visitorId, userTag);
                }
                return false;
            }
        });
