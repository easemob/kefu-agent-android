...
//        BADGERS.add(LGHomeBadger.class);
    ...
    public static boolean applyCount(Context context, Notification notification, int notificationId, int thisNotificationCount, int badgeCount) {
        try {
            applyCountOrThrow(context, notification, notificationId, thisNotificationCount, badgeCount);
            return true;
        } catch (ShortcutBadgeException e) {
            if (Log.isLoggable(LOG_TAG, Log.DEBUG)) {
                Log.d(LOG_TAG, "Unable to execute badge", e);
    ...
    public static void applyCountOrThrow(Context context, Notification notification, int notificationId, int thisNotificationCount, int badgeCount) throws ShortcutBadgeException {
        if (sShortcutBadger == null) {
            boolean launcherReady = initBadger(context);
        ...
        try {
            sShortcutBadger.executeBadge(context, sComponentName, notification, notificationId, thisNotificationCount, badgeCount);
        } catch (Exception e) {
            throw new ShortcutBadgeException("Unable to execute badge", e);
        }
    }
    ...
    private static boolean initBadger(Context context) {
        ...
        if (sShortcutBadger == null) {
            if (Build.MANUFACTURER.equalsIgnoreCase("Xiaomi"))
                ...
                sShortcutBadger = new NewHtcHomeBadger();
            else
